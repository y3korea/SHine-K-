...
- `prototype_worksite/` — **runnable worksite edge module** backing the Working-status claims: in-browser MoveNet MultiPose pose + fall state machine + a two-cue posture-strain index (a REBA-inspired proxy, not a REBA score), and a real YOLOv11 **PPE** detector (`ppe.onnx`) via ONNX Runtime Web. Serve the folder and open `worksite_multi.html` with a webcam
- `latency_benchmark.js` — post-processing micro-benchmark (two-cue posture-strain index + fire pixel-scan; neural inference excluded)

...
// warmup
timeit(()=>reba(lm),20000); timeit(()=>fire(img,W,H),5000);
const N=50000;
const rb=timeit(()=>reba(lm),N);
const fr=timeit(()=>fire(img,W,H),20000);
const node=process.version;
const rebaMs=rb.ms, fireMs=fr.ms;
console.log("Node "+node);
console.log("REBA 각도/위험 산출:  "+rebaMs.toFixed(4)+" ms/frame  → 이론상 "+Math.round(1000/rebaMs).toLocaleString()+" fps 상한");
console.log("화재 픽셀분석(80x60): "+fireMs.toFixed(4)+" ms/frame  → 이론상 "+Math.round(1000/fireMs).toLocaleString()+" fps 상한");
console.log("후처리 합계:          "+(rebaMs+fireMs).toFixed(4)+" ms/frame");
